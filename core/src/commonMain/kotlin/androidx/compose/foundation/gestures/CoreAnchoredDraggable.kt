...
/**
 * Structure that represents the anchors of a [CoreAnchoredDraggableState].
 *
 * See the DraggableAnchors factory method to construct drag anchors using a default implementation.
 */
internal interface CoreDraggableAnchors<T> {
...
/**
 * [CoreDraggableAnchorsConfig] stores a mutable configuration anchors, comprised of values of [T] and
 * corresponding [Float] positions. This [CoreDraggableAnchorsConfig] is used to construct an immutable
 * [CoreDraggableAnchors] instance later on.
 */
internal class CoreDraggableAnchorsConfig<T> {
...
/**
 * Create a new [CoreDraggableAnchors] instance using a builder function.
 *
 * @param builder A function with a [CoreDraggableAnchorsConfig] that offers APIs to configure anchors
 * @return A new [CoreDraggableAnchors] instance with the anchor positions set by the `builder`
 * function.
 */
internal fun <T : Any> CoreDraggableAnchors(
    builder: CoreDraggableAnchorsConfig<T>.() -> Unit
): CoreDraggableAnchors<T> = MapCoreDraggableAnchors(CoreDraggableAnchorsConfig<T>().apply(builder).anchors)

/**
 * Enable drag gestures between a set of predefined values.
 *
 * When a drag is detected, the offset of the [CoreAnchoredDraggableState] will be updated with the drag
 ...
 * reached, the value of the [CoreAnchoredDraggableState] will also be updated to the value
 * corresponding to the new anchor.
 *
 * Dragging is constrained between the minimum and maximum anchors.
 *
 * @param state The associated [CoreAnchoredDraggableState].
 * @param orientation The orientation in which the [coreAnchoredDraggable] can be dragged.
 * @param enabled Whether this [coreAnchoredDraggable] is enabled and should react to the user's input.
 ...
 */
internal fun <T> Modifier.coreAnchoredDraggable(
    state: CoreAnchoredDraggableState<T>,
...
/**
 * Scope used for suspending anchored drag blocks. Allows to set [CoreAnchoredDraggableState.offset] to
 * a new value.
 *
 * @see [CoreAnchoredDraggableState.anchoredDrag] to learn how to start the anchored drag and get the
 * access to this scope.
 */
internal interface CoreAnchoredDragScope {
    /**
     * Assign a new value for an offset value for [CoreAnchoredDraggableState].
     *
     * @param newOffset new value for [CoreAnchoredDraggableState.offset].
...
/**
 * State of the [coreAnchoredDraggable] modifier.
 ...
 */
@Stable
internal class CoreAnchoredDraggableState<T>(
    initialValue: T,
    ...
    /**
     * Construct an [CoreAnchoredDraggableState] instance with anchors.
     *
     * @param initialValue The initial value of the state.
    ...
    internal constructor(
        initialValue: T,
        anchors: CoreDraggableAnchors<T>,
    ...
    internal val draggableState = object : DraggableState {
        ...
        override suspend fun drag(
            ...
            this@CoreAnchoredDraggableState.anchoredDrag(dragPriority) {
                with(dragScope) { block() }
            }
        }

        override fun dispatchRawDelta(delta: Float) {
            this@CoreAnchoredDraggableState.dispatchRawDelta(delta)
        }
    }

    /**
     * The current value of the [CoreAnchoredDraggableState].
     */
     ...
     * bounds, or 1f if the [CoreAnchoredDraggableState] is in a settled state.
     */
//    @get:FloatRange(from = 0.0, to = 1.0)
    ...
    var anchors: CoreDraggableAnchors<T> by mutableStateOf(emptyDraggableAnchors())
        private set
     ...
     */
    fun updateAnchors(
        newAnchors: CoreDraggableAnchors<T>,
    ...
    private val coreAnchoredDragScope: CoreAnchoredDragScope = object : CoreAnchoredDragScope {
     ...
     * anchors.
     *
     * All actions that change the [offset] of this [CoreAnchoredDraggableState] must be performed
    ...
    suspend fun anchoredDrag(
        ...
        block: suspend CoreAnchoredDragScope.(anchors: CoreDraggableAnchors<T>) -> Unit
    ) {
        try {
            dragMutex.mutate(dragPriority) {
                restartable(inputs = { anchors }) { latestAnchors ->
                    coreAnchoredDragScope.block(latestAnchors)
                }
            }
        } finally {
     ...
     * to arrive to. This will set [CoreAnchoredDraggableState.targetValue] to provided value so
     * consumers can reflect it in their UIs.
     *
     * <b>If the [anchors] or [CoreAnchoredDraggableState.targetValue] change while the [block] is being
    ...
    suspend fun anchoredDrag(
        targetValue: T,
        dragPriority: MutatePriority = MutatePriority.Default,
        block: suspend CoreAnchoredDragScope.(anchors: CoreDraggableAnchors<T>, targetValue: T) -> Unit
    ) {
        if (anchors.hasAnchorFor(targetValue)) {
            try {
                dragMutex.mutate(dragPriority) {
                    ...
                    restartable(
                        inputs = { anchors to this@CoreAnchoredDraggableState.targetValue }
                    ) { (latestAnchors, latestTarget) ->
                        coreAnchoredDragScope.block(latestAnchors, latestTarget)
                    }
                }
            } finally {
                dragTarget = null
    ...
    /**
     * Drag by the [delta], coerce it in the bounds and dispatch it to the [CoreAnchoredDraggableState].
     *
     * @return The delta the consumed by the [CoreAnchoredDraggableState]
     */
    ...
    companion object {
        /**
         * The default [Saver] implementation for [CoreAnchoredDraggableState].
         */
        ...
        ) = Saver<CoreAnchoredDraggableState<T>, T>(
            save = { it.currentValue },
            restore = {
                CoreAnchoredDraggableState(
                    initialValue = it,
 ...
 * If the [targetValue] is not in the set of anchors, the [CoreAnchoredDraggableState.currentValue] will
 ...
 */
internal suspend fun <T> CoreAnchoredDraggableState<T>.snapTo(targetValue: T) {
 ...
 */
internal suspend fun <T> CoreAnchoredDraggableState<T>.animateTo(
    targetValue: T,
...
private class CoreAnchoredDragFinishedSignal : CorePlatformOptimizedCancellationException()
        ...
        coroutineScope {
            ...
            snapshotFlow(inputs)
                .collect { latestInputs ->
                    previousDrag?.apply {
                        cancel(CoreAnchoredDragFinishedSignal())
                        join()
                    }
                    previousDrag = launch(start = CoroutineStart.UNDISPATCHED) {
                        block(latestInputs)
                        this@coroutineScope.cancel(CoreAnchoredDragFinishedSignal())
                    }
                }
        }
    } catch (anchoredDragFinished: CoreAnchoredDragFinishedSignal) {
        // Ignored
    }
}

private fun <T> emptyDraggableAnchors() = MapCoreDraggableAnchors<T>(emptyMap())

private class MapCoreDraggableAnchors<T>(private val anchors: Map<T, Float>) : CoreDraggableAnchors<T> {
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (other !is MapCoreDraggableAnchors<*>) return false
