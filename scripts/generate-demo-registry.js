...
function parseDemoArguments(args, fail) {
  ...
  const enumValue = (name, type, fallback) => {
    const value = argumentsByName.get(name);
    if (!value) return fallback;
    if (value.length !== 3 || value[0] !== type || value[1] !== '.' || !/^[A-Z][A-Za-z0-9]*$/.test(value[2])) {
      fail(`${name} must be a ${type} value`);
    }
    return value[2];
  };
  ...
  return {
    ...
    section: enumValue('section', 'DemoSection', 'Components'),
    contentAlignment: enumValue('contentAlignment', 'DemoContentAlignment', 'Center'),
    padding: enumValue('padding', 'DemoPadding', 'Default'),
  };
}
